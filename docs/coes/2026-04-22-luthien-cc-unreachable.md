...
| Outstanding Claude-automatable work needs a home so it gets picked up next session | Added action items | `dev/TODO.md` |
...
|--------|-------|-----|------|
| Audit every reference to `luthien.cc` across this repo, the pitch-deck sources, `personal-site`, `luthien-org`, email signatures found in the gdrive clone, and the LOI tracking sheet. Produce a swap-list grouped by surface, so the canonical-domain cutover is a mechanical edit rather than a scavenger hunt. In-repo sweep already done above; cross-repo sweep pending. | Claude | Next session | **Point fix (this instance) + Detection (surfaces the surface area)** |
| Edit the in-repo self-references once canonical domain is picked: OG tags in `site/index.html`, `site/about.html`, `site/blog.html`; text in `site/assets/images/og-image.svg`; close-slide links in `site/pitch/index.html` and `site/pitch/es/index.html`; references in `site/install.sh` and CHANGELOG. Open as a separate PR (one-PR-one-concern) once the decision is made. | Claude | After canonical-domain decision | **Point fix** |
| Build a reachability probe script checked in at `scripts/reachability-check.sh` that curls the domain from several external HTTP-probe services (e.g. uptimerobot-style free probes, or multiple residential-IP VPN exits) and reports pass/fail + failure signature per probe. Run it against both `luthien.cc` and the eventual canonical domain. | Claude | Next session | **Architectural (prevents the class)** |
| Spot-check email deliverability: send one email from `@luthien.cc` and `@luthienresearch.org` into a Gmail, Outlook, and Yahoo test inbox; report inbox/spam placement. If `@luthien.cc` lands in spam, raise that as a separate surface to cut over. | Claude | Next session | **Detection (shortens exposure window)** |

*Requires human decision/design:*

| Action | Owner | Due | Type |
|--------|-------|-----|------|
| Decide canonical Luthien domain. Options on the table: keep `luthien.ai` (already works, already owned) as canonical; acquire `luthien.com` if available; pick a different `.io`/`.dev`/etc. Decision should weigh TLD reputation, price, and brand. | Jai + Scott | 2026-04-29 | **Architectural** |
| Set up a 301 redirect `luthien.cc` → canonical (via Cloudflare Page Rules or equivalent). Caveat: users on ISPs that block the TLS handshake to luthien.cc will never reach the redirect; those users stay broken until they update their bookmarks. The redirect is still worth setting up for the subset of ISPs that only filter HTTP or do not filter at all. | Jai | Same day as decision | **Point fix** |
| Add a written domain-selection checklist to `dev/context/decisions.md` or a new `docs/process/domain-selection.md`. Must include: TLD reputation (Spamhaus, VirusTotal, Google Safe Browsing), residential-ISP reachability check, shared-IP neighborhood spot-check, email-deliverability spot-check. Run before promoting any new domain externally. | Jai or Scott | Before next domain purchase | **Architectural (prevents the class)** |
| Decide whether to stand up ongoing synthetic monitoring for all customer-facing domains. Cheapest version: free UptimeRobot or Hyperping probes from 3+ regions. | Jai | Next sprint | **Detection** |
...
A second meta-observation, surfaced by the sibling process COE filed alongside this one: the `/coe` process itself almost did not produce this document. The first attempt at running `/coe` on this incident produced an inline chat response with no PR, no committed files, and no registry entry. Process-level adherence to COE is itself a class of bug, and is addressed in the sibling COE.
