...
| **R2** | <u>Logos over faces for credibility.</u> An investor who doesn't know the angels personally gets credibility from *where they came from*, not from seeing their face. Company affiliation logos are the primary credential. | v6 shows face → name → text credential. Only 4 ex-Amazon folks have a logo visible; the other 5 have text-only affiliations. No R2/HelloFresh/SeekOut/Sproutron/Habitat Health logos in repo. | [A2](#a2-esben-on-logos-as-the-credential). |
...
Open questions I need Scott to resolve before re-implementing:

1. **Faces yes or no?** R1 (minimize overwhelm) + R2 (logos > faces) points toward no faces. Scott's Apr 11 note "keep the faces, but for notable people include relevant stuff" points toward yes faces. **These conflict.** The 5-concept mockup was built because I couldn't resolve this. Needs a decision.
2. **How many angels to name?** R6 says "one credibility anchor minimum." R2 says "logos over faces." R4 says "show both recency and affiliation." Showing all 9 with logos may satisfy R4 at the cost of R1. Showing only Guillermo + a count satisfies R1+R6 but loses R4's recency dimension.
3. **Are missing logos a hard blocker?** Only Amazon is in repo. R2 (the YC company), HelloFresh, SeekOut, Sproutron AI, Habitat Health, LIFE In The Air are all missing. Fetching them is ~20 minutes but pulls the investor emailout further into tonight.
4. **Should we name grant funders as text (R8's "acceptable" path) or cut them entirely?** Esben said either works; Scott's preference not yet recorded.
5. **Does Guillermo's title need "YC W21, acq. by Ant International" verbatim?** R6 only demands "most impressive title." The full acquisition detail strengthens credibility but lengthens the credential line.
